...
| <a name="input_is_accessible_from_bastion"></a> [is\_accessible\_from\_bastion](#input\_is\_accessible\_from\_bastion) | **Add the Virtual Machine to the bastion.**<br>  - Constraint:<br>  Valid values for is\_accessible\_from\_bastion are [true \| false]<br>    If `is_accessible_from_bastion = true`, `bastion_allowed_ba_entities` value must be provided to the module.<br>    If `is_accessible_from_bastion = true` and ad_domain != "workgroup", `bastion_allowed_ad_entities` or `bastion_allowed_ad_groups` value must be provided to the module.<br>  - Example:<pre>ad_domain = "DomainName"<br>is_accessible_from_bastion = true<br>bastion_allowed_ba_entities = "BastionAccount1,BastionAccount2"<br>bastion_allowed_ad_entities = "ADAccount1,ADAccount1"</pre> | `bool` | `false` | no |
